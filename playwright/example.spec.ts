...
test.describe("Todo app API testing", () => {
    ...
    test.beforeAll(async () => {
        ...
        await page.evaluate(({ baseURL, endPoint }) => {
    ...
    test("should be able to add a todo", async ({ page }) => {
        const response = await page.evaluate(({ baseURL, endPoint }) => {
            return fetch(baseURL + endPoint, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ title: "Buy milk", description: "Buy milk from the store" }),
            }).then((res) => res.json());
        }, { baseURL, endPoint });
    ...
    test("should be able to update a todo", async ({ page }) => {
        ...
        const updateResponse = await page.evaluate(({ baseURL, endPoint, todoId }) => {
            return fetch(baseURL + endPoint + `/${todoId}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ title: "Buy juice", description: "Buy juice from the amazon" }),
            }).then((res) => res.json());
